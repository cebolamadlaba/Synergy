    ...
    public class ConsessionNotificationJob : IJob
    {
        ...
        public async Task Run()
        {
            var query = @"
                ...
                ";
            var concessions =
                await connection.QueryAsync<ConsessionNotification>(query, new {date = DateTime.Today.AddMonths(3)});
            if (concessions.Any())
            {
                concessions.GroupBy(x => x.EmailAddress).ToList().ForEach(g =>
                {

                    emailManager.SendTemplatedEmail(g.Key, "Notification: Expiring Concession(s)", string.Empty,
                        "ExpiringConcession",
                        new {Concessions = g.Select(x => x.ConcessionRef).ToList(), Name = g.First().FirstName});
